              ...
              <div class="d-lg-flex">
                ...
                <div class="ms-auto my-auto mt-lg-0 mt-4">
                  <div class="ms-auto my-auto">
                    <a href="{{ route('admin.users.create') }}" class="btn bg-gradient-primary btn-sm mb-0">+&nbsp; {{ __('New User')}}</a>
                ...
                <table class="table table-flush" id="products-list">
                  ...
                  <tbody>
                    @foreach ($users as $user)
                      <tr>
                        <td><a href="{{ route('admin.users.show',$user->id) }}" ><p class="ms-3 my-2 text-sm" style="font-weight: bolder">{{ $user->id }}</p></a></td>
                        <td class="w-12">
                          <div class="d-flex">
                            {{-- $user->profile_photo_path --}}
                            <img class="w-10 rounded-full" src="https://raw.githubusercontent.com/creativetimofficial/public-assets/master/soft-ui-design-system/assets/img/ecommerce/adidas-hoodie.jpg" alt="hoodie">
                            <h6 class="my-2 ml-2 font-extrabold">{{ $user->name." ".$user->username }} </h6>
                          </div>
                        </td>
                        <td><p class="ms-3 my-2 text-sm">{{ $user->email }}</p></td>
                        <td><p class="ms-3 my-2 text-sm">{{ $user->phone }}</p></td>
                        <td><p class="ms-3 my-2 text-sm">{{ $user->country }}</p></td>
                        <td><p class="ms-3 my-2 text-sm">{{ $user->company }}</p></td>
                        <td>
                          <span class="badge badge-success badge-sm ms-3 my-2">{{$user->active == 1 ? 'Active' : 'Not active'}}</span>
                        </td>
                        <td>
                          <div class="d-flex ms-3 my-2 text-sm">
                          ...
                          <form action="{{ route('admin.users.destroy',$user->id)}}"  method="POST">
                            @method('DELETE')
                            @csrf
                            <button  class="mx-1" type="submit" data-bs-toggle="tooltip" data-bs-original-title="Delete user" style="border:none; background-color:transparent;padding-left:0"><i class="fas fa-trash text-danger"></i></button>
                          </form>
                        </div>
                        </td>
                      </tr>
